...
# --- 1. Definiranje Custom Callbacka za praćenje performansi ---
class TrainAndEvalCallback(BaseCallback):
    ...
    def __init__(self, eval_env, eval_freq, n_eval_episodes=10, verbose=1):
        ...
        self.timesteps = []
        # Postavljamo best_mean_reward na negativnu beskonačnost kako bi osigurali da se prvi model spasi
        self.best_mean_reward = -np.inf 

    def _on_step(self) -> bool:
        """
        Poziva se na svakom koraku (timestep) treninga.
        """
...
## 2. Postavke za trening

# Postavke za trening
timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
LOG_DIR = f"./logs/frozenlake_policy_gradient_logs_{timestamp}/"
...
# Rjecik za pohranu callback podataka
performance_data = {}

print(f"Trening FrozenLake-v1 agenata će se spremati u direktorij: {LOG_DIR}")

## 3. Treniranje A2C agenta

print("\n--- Pokrećem trening A2C agenta ---")

# is_slippery=True je defaultno za FrozenLake-v1, ali eksplicitno ga navodimo radi jasnoce.
env_a2c = gym.make("FrozenLake-v1", is_slippery=True)
eval_env_a2c = Monitor(gym.make("FrozenLake-v1", is_slippery=True))
...
model_a2c = A2C(
    ...
    learning_rate=0.0007, # Defaultna vrijednost za A2C
    gamma=0.99,
)
...
model_a2c.save(os.path.join(LOG_DIR, "a2c_frozenlake"))
env_a2c.close()
eval_env_a2c.close()
performance_data['A2C'] = {
    'timesteps': callback_a2c.timesteps,
...
print(f"A2C model spremljen u: {os.path.join(LOG_DIR, 'a2c_frozenlake.zip')}")
print(f"A2C trening vrijeme: {format_time(training_time_a2c_minutes)}")

## 4. Treniranje PPO agenta
print("\n--- Pokrećem trening PPO agenta ---")
env_ppo = gym.make("FrozenLake-v1", is_slippery=True)
eval_env_ppo = Monitor(gym.make("FrozenLake-v1", is_slippery=True))
...
model_ppo = PPO(
    ...
    verbose=0,
    tensorboard_log=LOG_DIR,
    learning_rate=0.0003, # Defaultna vrijednost, često dobra za PPO
    gamma=0.99,
    n_steps=2048, 
    clip_range=0.2 
)
...
model_ppo.save(os.path.join(LOG_DIR, "ppo_frozenlake"))
env_ppo.close()
eval_env_ppo.close()
performance_data['PPO'] = {
    'timesteps': callback_ppo.timesteps,
...
print(f"PPO model spremljen u: {os.path.join(LOG_DIR, 'ppo_frozenlake.zip')}")
print(f"PPO trening vrijeme: {format_time(training_time_ppo_minutes)}")

## 5. Spremanje podataka o performansama u JSON datoteku
...
print("\n--- Trening FrozenLake-v1 agenata završen. ---")
print(f"Spremljeni modeli i podaci nalaze se u direktoriju: {LOG_DIR}")
print("Sada možete koristiti skriptu za vizualizaciju ili finalnu evaluaciju modela.")