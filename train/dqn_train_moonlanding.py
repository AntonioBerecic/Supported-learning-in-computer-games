...
def train():
    ...
    plt.title("Deep Q-learning na LunarLander-v3")
    plt.xlabel("Epizoda")
    plt.ylabel("Ukupna nagrada")

    plt.plot(rewards_history, color='lightgray', label="Originalna nagrada", alpha=0.3)
    plt.plot(rolling_mean, color='purple', label=f"Prosječna nagrada ({window} epizoda)")
    plt.fill_between(range(len(rewards_history)),
                     rolling_mean - rolling_std,
                     rolling_mean + rolling_std,
                     color='purple', alpha=0.2, label="Standardna devijacija")

    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig("logs/dqn_moonlander_learning_curve.png")
    plt.close()
